...
async function analyzeWithLLM(tasks, apiKey) {
...
function fallbackAnalysis(tasks) {
  const keywords = {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!tasks || !Array.isArray(tasks) || tasks.length < 2) {
      return res.status(400).json({ error: "Provide at least 2 tasks as an array of strings." });
    }
    if (tasks.length > 10) {
      return res.status(400).json({ error: "Maximum 10 tasks per analysis." });
    }
